package com.hh.wx.v3.sp_product;
...
@AllArgsConstructor
@Log4j2
public class Wx3SpJsapiService implements PayV3Product {

    private Wx3Constant wx3Constant;
    private OrderCheck orderCheck;

    private static final String HEAD = "[微信支付V3]-[小程序支付]";
    ...
    public Map<String, String> placeOrder(PayReqVO reqVO) {
        ...
        payer.setSpOpenid(reqVO.getOpenid());
        request.setPayer(payer);
        request.setSubMchid(reqVO.getSubMchId());
        log.info(HEAD + "统一下单,入参:{}", JSON.toJSONString(request));
        // 调用下单方法，得到应答
        PrepayWithRequestPaymentResponse response = service.prepayWithRequestPayment(request, wx3Constant.getAppid());
        log.info(HEAD + "统一下单,出参:{}", JSON.toJSONString(response));
        return JSON.parseObject(JSON.toJSONString(response), Map.class);
    }
